...
unsigned int period_timeRXTX = 42666;               			/* period time in us */
...
int addhelpRXTX=0;												/* Print usage*/
static snd_pcm_format_t format = SND_PCM_FORMAT_S16_LE; 	/* sample format */

// TX PARAMS
snd_pcm_t *handleTX;
static char *deviceTX = "plughw:0,0";                     	/* playback device */
/* Pointer to ALOE input/output buffers*/
_Complex float *datoutputTX;
// ALSA TX parameters
double freqTX = 999.0;                              	 	/* sinusoidal wave frequency in Hz */
signed short *samplesTX;
snd_pcm_channel_area_t *areasTX;

// RX PARAMS
snd_pcm_t *handleRX;
static char *deviceRX = "plughw:0,0";  					  	/* capture device*/
/* Pointer to ALOE input/output buffers*/
_Complex float *datinputRX;

// ALSA RX parameters
signed short *samplesRX;
snd_pcm_channel_area_t *areasRX;
...
static int async_loop(snd_pcm_t *handleRX, signed short *samplesRX, snd_pcm_channel_area_t *areasRX, 
					  snd_pcm_t *handleTX, signed short *samplesTX, snd_pcm_channel_area_t *areasTX);
static void help(void);

int init_toneCOMPLEXRXTX(_Complex float *table, int length, float ref_freq, float gain, float sampl_freq);
int gen_toneCOMPLEXRXTX(_Complex float *func_out, _Complex float *tablel, int tablesz, 
					int datalen, float ref_freq, float tone_freq, float sampl_freq);
void CPLXconvert2pcm8bitsstreamRXTX(_Complex float *in, signed char *out, int numframesIN);
void CPLXconvert2pcm16bitsstreamRXTX(_Complex float *in, signed short *out, int numframesIN);
void pcm16bitsstream2CPLXconvertRXTX( signed short *in, _Complex float *out, int numframesIN);
...
static void async_callbackRX(snd_async_handler_t *ahandler)
{
	int i;	
	static int Tslot=0;
	...
	while (avail >= period_size){
		...
		pcm16bitsstream2CPLXconvertRXTX(samples, soundsignal, (int)block_size);
        if (err < 0) {
...
static void async_callbackTX(snd_async_handler_t *ahandler){
    ...
    int err;
	_Complex float *soundsignal;
	signed short value = 12000;
        
 	soundsignal=datoutputTX;
	avail = snd_pcm_avail_update(handle);
	while (avail >= period_size){
...
int alsacardRXTX_readcfg(struct dac_cfg *dac_cfg) {
	int i=0, j, offset;
	char *buffer;
	cfg_o cfg;
	sect_o sect;
...
//			dac_cfg->NsamplesOut = 1024;
		...
		period_timeRXTX=(unsigned int)((period_size*1000000)/rate);
...
int alsacardRXTX_init(struct dac_cfg *cfg, int *timeSlotLength, void (*sync)(void)) {

		/* Capture input/output buffer pointers*/
		datinputRX = (_Complex float *)&cfg->dacinbuff[0][0];
		datoutputTX = (_Complex float *)&cfg->dacoutbuff[0][0];

		syncFunction=sync;

		/* Print ALSA driver usage*/
		if(addhelpRXTX==1)help();

		/* Configure ALSA params*/
		snd_pcm_hw_params_alloca(&hwparams);
		snd_pcm_sw_params_alloca(&swparams);

		/* Calculate Time Slot according capture needs*/
		...
		*timeSlotLength=(int)(1000000.0*((float)(cfg->NsamplesIn)/(float)(SamplingFrequency)));	
		...
		// SETUP RX
        if ((err = snd_pcm_open(&handleRX, deviceRX, SND_PCM_STREAM_CAPTURE, 0)) < 0) {
		...
		// SETUP TX
      	if ((err = snd_pcm_open(&handleTX, deviceTX, SND_PCM_STREAM_PLAYBACK, 0)) < 0) {
                printf("Playback open error: %s\n", snd_strerror(err));
                return 0;
        }
        
      	if ((err = set_hwparams(handleTX, hwparams, SND_PCM_ACCESS_RW_INTERLEAVED)) < 0) {
                printf("Setting of hwparams failed: %s\n", snd_strerror(err));
                exit(EXIT_FAILURE);
        }


        if ((err = set_swparams(handleTX, swparams)) < 0) {
                printf("Setting of swparams failed: %s\n", snd_strerror(err));
                exit(EXIT_FAILURE);
        }
       	if (verbose > 0)snd_pcm_dump(handleTX, output);

		// CREATE MEMORY ZONE 
        samplesRX = malloc((period_size * channels * snd_pcm_format_physical_width(format)) / 8);
        if (samplesRX == NULL) {
                printf("No enough memory\n");
                return -1;
        }     
       	samplesTX = malloc((period_size * channels * snd_pcm_format_physical_width(format)));// / 8);
        if (samplesTX == NULL) {
                printf("No enough memory\n");
                return -1;
        }
        areasRX = calloc(channels, sizeof(snd_pcm_channel_area_t));
        if (areasRX == NULL) {
                printf("No enough memory\n");
                return -1;
        }
		areasTX = calloc(channels, sizeof(snd_pcm_channel_area_t));
        if (areasTX == NULL) {
                printf("No enough memory\n");
                return -1;
        }
        for (chn = 0; chn < channels; chn++) {
                areasRX[chn].addr = samplesRX;
                areasRX[chn].first = chn * snd_pcm_format_physical_width(format);
                areasRX[chn].step = channels * snd_pcm_format_physical_width(format);
        }
      	for (chn = 0; chn < channels; chn++) {
                areasTX[chn].addr = samplesTX;
                areasTX[chn].first = chn * snd_pcm_format_physical_width(format);
                areasTX[chn].step = channels * snd_pcm_format_physical_width(format);
        }



		async_loop(handleRX, samplesRX, areasRX, 
					  handleTX, samplesTX, areasTX);
        if (err < 0)
                        ...
                        snd_pcm_access_t access)
{
        ...
        buffer_size = size;
        /* set the period time */
        err = snd_pcm_hw_params_set_period_time_near(handle, params, &period_timeRXTX, &dir);
        if (err < 0) {
                printf("Unable to set period time %i for playback: %s\n", period_timeRXTX, snd_strerror(err));
                return err;
        }
...
static int async_loop(snd_pcm_t *handleRX, signed short *samplesRX, snd_pcm_channel_area_t *areasRX, 
					  snd_pcm_t *handleTX, signed short *samplesTX, snd_pcm_channel_area_t *areasTX)
{
        struct async_private_data dataRX, dataTX;
        snd_async_handler_t *ahandlerRX, *ahandlerTX;
        int err, count;
        dataRX.samples = samplesRX;
        dataRX.areas = areasRX;
        dataRX.phase = 0;
        dataTX.samples = samplesTX;
        dataTX.areas = areasTX;
        dataTX.phase = 0;


        err = snd_async_add_pcm_handler(&ahandlerRX, handleRX, async_callbackRX, &dataRX);
        if (err < 0) {
                printf("Unable to register async handler RX\n");
                exit(EXIT_FAILURE);
        }

       	err = snd_async_add_pcm_handler(&ahandlerTX, handleTX, async_callbackTX, &dataTX);
        if (err < 0) {
                printf("Unable to register async handler TX\n");
                exit(EXIT_FAILURE);
        }
       for (count = 0; count < 2; count++) {
                generate_sine(areasTX, 0, period_size, &dataTX.phase);
                err = snd_pcm_writei(handleTX, samplesTX, period_size);
                if (err < 0) {
       ...
       if (snd_pcm_state(handleRX) == SND_PCM_STATE_PREPARED) {
                err = snd_pcm_start(handleRX);
                if (err < 0) {
                        printf("Start error: handler RX%s\n", snd_strerror(err));
                        exit(EXIT_FAILURE);
                }
        }
       if (snd_pcm_state(handleTX) == SND_PCM_STATE_PREPARED) {
                err = snd_pcm_start(handleTX);
                if (err < 0) {
                        printf("Start error: handle TX%s\n", snd_strerror(err));
                        exit(EXIT_FAILURE);
                }
        }
                          ...
                          int count, double *_phase)
{
        ...
        double phase = *_phase;
        double step = max_phase*freqTX/(double)rate;
