...
_Complex float CBuffer_IN[CBUFFER_SZ], CBuffer_OUT[CBUFFER_SZ];
buffctrl in_buffCtrl, out_buffCtrl;
_Complex float datout[64*1024];
int NsamplesIN=0, NsamplesOUT=0;
...
int process(jack_nframes_t nframes, void *arg) {
	...
	int i;
	static int j=0, k=0, m=0, count=0;
...
//	printf("JACK process\n");
	...
	//MODE NORMAL
/*	if(exec_mode==NORMAL){
		// Write to the Output CBuffer
		if(m==0)writeCbuff(&out_buffCtrl, CBuffer_OUT, output, NsamplesOUT); 	//Rebem nframes de NsamplesIn
		

//		printf("JACK process(): count=%d, NsamplesOUT=%d, nframes=%d\n", count, NsamplesOUT, nframes);

		
		readCbuff(&out_buffCtrl, CBuffer_OUT, datout, nframes);
		for(i=0; i<nframes; i++){
//			__real__ input[i]=in1[i];
//			__imag__ input[i]=in2[i];
			out1[i]=__real__ datout[i];
			out2[i]=__imag__ datout[i];
		}
	}
*/
	...
	if(m==1)m=0;
	count++;
	if(count == 1000)exec_mode=NORMAL;
	syncFunction();
	return 0;
}
...
printf("A2");
	...
	if (jack_set_buffer_size(client,1024)) {
...
printf("A3");
/*	if (jack_set_sample_rate_callback(client,(int)(cfg->inputFreq), 0)) {
		 printf("ERROR setting sampling rate to %d. It should not excess 48000\n",cfg->inputFreq);
		 printf("Please, correct the samp_freq value at hw_api/lnx/cfg/dacsound.conf file currently being used\n");
		 return -1;
	 } 
*/
printf("B");	
