...
/**
  ...
  @param	h Gnuplot session control handle.
  @param	label Character string to use for title.
  @return	void
...
void gnuplot_line_color(gnuplot_ctrl * h, char * label)
{
    char    cmd[GP_CMD_SIZE] ;

    sprintf(cmd, "set style line 1 lc rgb \"%s\" lt 1 lw 1.5", label) ;
    gnuplot_cmd(h, cmd) ;
    return ;
}
...
/*-------------------------------------------------------------------------*/
/**
  @brief	Sets the background color of a gnuplot session.
  @param	h Gnuplot session control handle.
  @param	color.
  @return	void
...
void gnuplot_set_background(gnuplot_ctrl * h, char * label)
{
    char    cmd[GP_CMD_SIZE] ;

    sprintf(cmd, "set object 1 rectangle  from screen 0,0 to screen 1,1 fillcolor rgbcolor \"%s\" behind", label) ;
    gnuplot_cmd(h, cmd) ;
    return ;
}
...
/*-------------------------------------------------------------------------*/
/**
  @brief    Sets the maximums and minimums of x and y axis.
  @param    h Gnuplot session control handle.
  @return   void
                  
  Sets the maximum and minimum of x and y axis. It sets the view range of
  the plot.
*/
/*--------------------------------------------------------------------------*/
void gnuplot_set_xy_range(gnuplot_ctrl * h, double xmin, double xmax, double ymin, double ymax){
    char    cmd[GP_CMD_SIZE] ;
    
    sprintf(cmd, "set xrange [%f:%f]; set yrange [%f:%f]", xmin, xmax, ymin, ymax);
    gnuplot_cmd(h, cmd) ;
    return ;
}
...
/**
  ...
  @code
    ...
    gnuplot_plot_xy(h, x, y, z, 50, "parabola") ;
    sleep(2) ;
    gnuplot_close(h) ;
  @endcode
 */
