#!/usr/bin/perl

$modulename = $ARGV[0];


$makefile = "Makefile.am";
$base_subdirs = "utils/lnx_make hw_api/lnx sw_daemons/lnx_make cmdman_console sw_api/lnx_make modules/typetools/lnx_make ";
$config = "configure.in";
$base_config = "AC_CONFIG_FILES([Makefile\n\tutils/lnx_make/Makefile\n\thw_api/lnx/Makefile\n\tsw_daemons/lnx_make/Makefile\n\tsw_api/lnx_make/Makefile\n\tcmdman_console/Makefile\n\tmodules/typetools/lnx_make/Makefile\n\t";
$base_config_cnt = 6;

$modules_dir = "modules/";
$make_append = "lnx_make/";

if ($modulename eq "") {
	print "Usage: add_module.pl module_name\n";
	exit;
}

if (-d "modules/".$modulename) {
	print "Module " . $modulename . " already exist.\n";
	exit;
}
print "Creating directory structure for module " . $modulename . "...\n";
system("cp -fr scripts/module_template.ALOE1.5 modules/");
system("mv modules/module_template.ALOE1.5 modules/".$modulename);
system("mv modules/".$modulename."/src/modulename_imp.c modules/".$modulename."/src/".$modulename."_imp.c");
system("mv modules/".$modulename."/src/modulename.h modules/".$modulename."/src/".$modulename.".h");

open(M,"modules/".$modulename."/lnx_make/Makefile.am");
@lines = <M>;
close(M);

$output="";
foreach $line (@lines) {
	$line =~ s/modulename/$modulename/g;
	$output .= $line;
}
open(M,">","modules/".$modulename."/lnx_make/Makefile.am");
print M $output;
close(M);

open(M,"modules/".$modulename."/src/".$modulename."_imp.c");
@lines = <M>;
close(M);

$output="";
foreach $line (@lines) {
	$line =~ s/modulename/$modulename/g;
	$output .= $line;
}
open(M,">","modules/".$modulename."/src/".$modulename."_imp.c");
print M $output;
close(M);

$i=0;

@modules = <modules/*>;

print "Adding the module to " . $makefile . " and " . $config . " files.\n";
foreach $module (@modules) {
	if ($module =~ m/typetools/) {
            $j=$i;
	}
	$i++;
}

splice(@modules,$j,1);

open(M,$makefile);
@lines = <M>;
close(M);

$output="";

$c=0;
foreach $line (@lines) {
	if ($line =~ m/^SUBDIRS/) {
		$output .= "SUBDIRS=" . $base_subdirs;
		foreach $module (@modules) {
			$output .= $module . "/" . $make_append . " ";
		}	
		$output .= "\n";
	} else {
		$output .= $line;
	}
}

open(M,">",$makefile);
print M $output;
close(M);

open(M,$config);
@lines = <M>;
close(M);

$output="";
$c=0;
foreach $line (@lines) {
	if ($line =~ m/^AC_CONFIG_FILES/ || ($c>0)) {
		if ($c==0) {
			$output .= $base_config;
			foreach $module (@modules) {
				$output .= "\t" . $module . "/" . $make_append . "Makefile\n";
			}		
			$output .= "\t])\n";
		}
		$c++;
		if ($line =~ m/\]\)/) {
			$c=0;
		}
	} else {
		$output .= $line;
	}
}
open(M,">",$config);
print M $output;
close(M);

print "Done.\n";



