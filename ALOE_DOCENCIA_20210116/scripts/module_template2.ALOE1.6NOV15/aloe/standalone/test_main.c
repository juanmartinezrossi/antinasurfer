...
#include "module_template2_params.h"
...
char plot_modeIN[128]="C1WLS";
char plot_modeOUT[128]="C2WLS";
...
void free_memory() {
	...
	if (parametersA) {
		for (int i=0;i<nof_params;i++) {
			if (parametersA[i].name) free(parametersA[i].name);FILE *dat_input=NULL, *dat_output=NULL;
			char *dat_input_name=NULL, *dat_output_name=NULL;
...
int read_BIN_file(char *input_name, FILE *input_pointer, int length, char *datatype, input_t *indata);
int read_TEXT_file(char *input_name, FILE *input_pointer, int length, char *datatype, input_t *indata);
int write_TEXT_file(char *output_name, FILE *p_output_file,
		output_t *out_data, int length, char *data_type);
...
//	printf("ctrl.inputMode=%s\n", ctrl.inputMode);
	...
	if(strcmp(ctrl.inputMode,"FILE_TEXT")==0){
		...
		if(nof_input_itf > 0){
			input_lengths[0]=ctrl.block_length*ctrl.run_times;
			z=read_TEXT_file(dat_input_name, dat_input, input_lengths[0], IN_TYPE, input_data);
			if(z==-1){
...
//	read_bin_file(dat_input_name, dat_input, int length, char *datatype);
	...
	if(strcmp(ctrl.inputMode,"FILE_BIN")==0){
		...
		if(nof_input_itf > 0){
			input_lengths[0]=ctrl.block_length*ctrl.run_times;
			z=read_BIN_file(dat_input_name, dat_input, input_lengths[0], IN_TYPE, input_data);
			if(z==-1){
				printf("Error reading input data text file\n");
				exit(1);
			}
...
//	printf("block_length=%d, run_times=%d,input_sample_sz=%d\n", ctrl.block_length, ctrl.run_times, input_sample_sz);
	...
	for (i=0;i<ctrl.run_times;i++) {
		ret = work(&input_data[i*ctrl.block_length], &output_data[out_length0]);
		out_length0+=ret;
	}
	input_lengths[0] = ctrl.block_length*ctrl.run_times;
...
//	printf("Nof nof_output_itf=%d\n", nof_output_itf);
	...
	printf("Output File oooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo\n");
//////////////////
	// Save put Text File
	// Open Output File
	if(strcmp(ctrl.outputMode,"FILE_TEXT")==0){
		write_TEXT_file(dat_output_name, dat_output, output_data,  output_lengths[0], OUT_TYPE);
	}
	if(strcmp(ctrl.outputMode,"FILE_BIN")==0){
		write_BIN_file(dat_output_name, dat_output, output_data,  output_lengths[0], OUT_TYPE);
	}
...
//				printf("WARNING!!! test_main(): input_lengths[%d]=%d\n", i, input_lengths[i]);
				...
				convert_data2Cfloat(input_data, plot_buffC, IN_TYPE, input_lengths[i]);
...
//				printf("WARNING!!! test_main(): output_lengths[%d]=%d\n", i, output_lengths[i]);
				...
				convert_data2Cfloat(output_data, plot_buffC, OUT_TYPE, output_lengths[i]);
...
int write_TEXT_file(char *output_name, FILE *p_output_file, output_t *out_data,  int length, char *data_type){
...
int write_BIN_file(char *output_filename, FILE *p_output_file, output_t *out_data,  int length, char *data_type){
